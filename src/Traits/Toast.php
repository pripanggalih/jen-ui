...
trait Toast
{
	public function toast(
		...
		$toast = [
			...
			'position' => $position,
			'icon' => Blade::render("<x-dynamic-component :component=\"config('jen.prefix', 'jen') . '::icon'\" class='w-7 h-7' name='" . $icon . "' />"),
			'css' => $css,
			'timeout' => $timeout,
		];
		...
		session()->flash('jen.toast.title', $title);
		session()->flash('jen.toast.description', $description);
